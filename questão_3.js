...
const faturamentoJson = [
  ...
  function processarFaturamento(dados) {
    const faturamentoDias = dados.filter(item => item.valor > 0).map(item => item.valor);
    ...
    const mediaMensal = somaFaturamento / faturamentoDias.length;
  
    const diasAcimaDaMedia = faturamentoDias.filter(valor => valor > mediaMensal).length;
